...
public interface WorkerUploadedDataRepository extends JpaRepository<WorkerUploadedData, Long> {
        ...
        List<WorkerUploadedData> findByFileIdAndStatusId(Long fileId, Integer statusId);

        Page<WorkerUploadedData> findByFileIdAndStatusId(Long fileId, Integer statusId, Pageable pageable);

        Page<WorkerUploadedData> findByFileId(Long fileId, Pageable pageable);

        @Query("SELECT COUNT(w) FROM WorkerUploadedData w WHERE w.fileId = :fileId AND w.statusId = :statusId")
        long countByFileIdAndStatusId(@Param("fileId") Long fileId, @Param("statusId") Integer statusId);

        @Query("SELECT w.statusId, COUNT(w) FROM WorkerUploadedData w WHERE w.fileId = :fileId GROUP BY w.statusId")
        ...
        Page<WorkerUploadedData> findByFileIdAndStatusIdAndCreatedAtBetween(Long fileId, Integer statusId,
        ...
        Page<WorkerUploadedData> findByCreatedAtBetween(java.time.LocalDateTime startDate,
